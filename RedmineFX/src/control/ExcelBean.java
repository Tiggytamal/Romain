...
public class ExcelBean implements Serializable, Instance
{
    ...
    public String charger(File file) throws IOException, EncryptedDocumentException, InvalidFormatException
    {
        // Récupération du fichier envoyé
        InputStream targetStream = FileUtils.openInputStream(file);

        // Création des deux workooks
        wbIn = WorkbookFactory.create(targetStream);
        wbOut = WorkbookFactory.create(targetStream);
    ...
    private List<Incident> avancement(Workbook wbIn, Workbook wbOut) throws GrowlException
    {
        ...
        if (compteIndex != 8)
            throw new GrowlException(Severity.SEVERITY_ERROR, "Erreur dans le format du fichier Excel", null);
    ...
    private void sm9(List<Incident> list, Workbook wbIn, Workbook wbOut) throws GrowlException
    {
        ...
        if (totalIndex != 13)
            throw new GrowlException(Severity.SEVERITY_ERROR, "Feuille Sm9 mal formatée", null);
